...
4. **Two entry points with different lifetimes.** `connect()` gives *this tab* a
   ...
   pages want `openSession`. Three methods are promises there because the work
...
6. **Two failures a page must not misread.** An ICE timeout is **not** evidence
   ...
   the effect twice.
7. **The package is built from the repository today.** `@net-mesh/browser` is not
   on a registry yet: build the leaf's wasm, then the package. Nothing here
   resolves the napi binding — that is the whole reason it is a sibling package.
...
2. **Pick the entry point before writing code.** `openSession()` for a real page
   ...
   or a page that is deliberately the one node. `openSession` takes
   `capabilities` and `subscriptions` **up front** so a new leader can restore
   them without being asked.
3. **Get the credential from the anchor, and know its scope.** `credentialB64`
   is signed and secret-bearing; the leaf presents it unmodified. An anchor's
   live verbs need the CLI's `rtc-bootstrap` feature — see `session.md`.
