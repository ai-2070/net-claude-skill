...
- **TS timestamps are `bigint`.** `Receipt.timestamp` and `StoredEvent.insertionTs` carry exact nanoseconds. They were `number`, which crossed `Number.MAX_SAFE_INTEGER` (≈ 9.0×10^15) at roughly **104 days past 1970** — so every realistic value had already lost its low-order digits, and `9007199254740993` read back as `...992`. Sub-microsecond latency deltas are now trustworthy. The cost is that **`JSON.stringify` throws on a `bigint`**: convert explicitly where you display or log, `Number(insertionTs / 1_000_000n)` for milliseconds. There is deliberately no second lossy `number` field to reach for.
