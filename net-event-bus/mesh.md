...
**As of 0.36 you still probably should not turn it on.** It ships dark: `enable_sensing_coalescing = false` by default, an origin additionally needs a persisted `sensing_incarnation`, the rendezvous-leader role is gated behind the `redex` build feature, and there is **no TS / Python / Go SDK surface**. A mesh that leaves the flag off is byte-for-byte the previous release on the wire; its two frames (`0x0C02` `SensingInterestFrame`, `0x0C03` `ReadinessAttestation`) are never emitted. What DOES exist in Rust: `mesh.sensing()?.provide(..)` (serve readiness for a capability) and `mesh.sensing()?.watch(..)` (observe your OWN organization's exact providers) — plus sensed provider selection underneath `OrgClient::call`, which needs no sensing code from you at all. The provider-free / leader arm is not lit in any language.
